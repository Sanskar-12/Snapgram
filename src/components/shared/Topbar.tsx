    ...
    <section className="topbar">
      <div className="flex-between py-4 px-5">
        ...
        <div className="flex gap-4">
          <Button variant={"ghost"} className="shad-button_ghost" onClick={signOutHandler}>
            <img src={"/assets/icons/logout.svg"} alt="Logout logo" />
          </Button>
          <Link to={`/profile/${user.id}`} className="flex-center gap-3">
            <img src={ user.imageUrl || "/public/assets/images/profile.png"} alt="img" className="h-8 w-8 rounded-full"/>
          </Link>
        </div>
      </div>
    </section>
